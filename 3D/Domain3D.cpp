...
bool Domain3D::updateBoxes(std::vector<Domain3D *> sideDomains, double minT) // XYYX, could change to other method?
{
    setGhostBoxes(sideDomains);
    if (!xUpdateBoxes(minT, sideDomains))
        return false;
    setGhostBoxes(sideDomains);
    if(!yUpdateBoxes(minT, sideDomains))
        return false;
    setGhostBoxes(sideDomains);
    if(!zUpdateBoxes(minT,sideDomains))
        return false;
    setGhostBoxes(sideDomains);
    if(!zUpdateBoxes(minT,sideDomains))
        return false;
...
bool Domain3D::xUpdateBoxes(double minT, std::vector<Domain3D *> sideDomains)
{
    if (!xFindFaces())
        return false;
...
bool Domain3D::yUpdateBoxes(double minT, std::vector<Domain3D *> sideDomains)
{
    if (!yFindFaces())
        return false;
...
bool Domain3D::zUpdateBoxes(double minT, std::vector<Domain3D *> sideDomains)
{
    if (!zFindFaces())
        return false;
    for (int x = 1; x < (xCellCount - 1); x++) // loop through all non edge boxes
