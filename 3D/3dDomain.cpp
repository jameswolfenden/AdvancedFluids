...
void saveDomain(Domain3D *domain, int num, int iteration, int z) // each domain is saved seperatly
{
    std::ofstream pCells, rhoCells, uCells, vCells, wCells;
    ...
    wCells.open("./results/" + std::to_string(num) + "wCells" + std::to_string(iteration) + ".csv");
    for (int y = 0; y < (domain->yCellCount); y++) // write final cell pressures
    {
        for (int x = 0; x < domain->xCellCount; x++)
        {
            ...
            {
                pCells << domain->boxes[x][y][z].p;
                rhoCells << domain->boxes[x][y][z].rho;
                uCells << domain->boxes[x][y][z].u;
                vCells << domain->boxes[x][y][z].v;
                wCells << domain->boxes[x][y][z].w;
            }
            else
            {
                pCells << domain->boxes[x][y][z].p << ",";
...
int main()
{
   ...
   // double fridgeHeight = 2.02; // full size
    ...
    int pipeDepthCells = (int) (pipeDepth*zCellsPerMetre);
    int frontFridgeDepthCells = (int) (frontFridgeDepth*zCellsPerMetre);
    int backFridgeDepthCells = (int) (backFridgeDepth*zCellsPerMetre);
    ...
    std::vector<Domain3D> domains = {
        Domain3D(pipeWidth, fridgeHeight, pipeDepth, pipeWidthCells, fridgeHeightCells, pipeDepthCells, domainsGhosts[0], 1, 1.3, 0.0, 0.0,0.0),  // centre fridge interior
        Domain3D(pipeWidth, pipeHeight, pipeDepth, pipeWidthCells, pipeHeightCells, pipeDepthCells, domainsGhosts[1], 1.1, 1.45, 0.0, 0.0, 0.0),        // pipe below fridge
        Domain3D(leftFridgeWidth, fridgeHeight, pipeDepth, leftFridgeWidthCells, fridgeHeightCells, pipeDepthCells, domainsGhosts[2], 1, 1.3, 0.0, 0.0, 0.0),  // left fridge interior (small)
        Domain3D(rightFridgeWidth, fridgeHeight, pipeDepth, rightFridgeWidthCells, fridgeHeightCells, pipeDepthCells, domainsGhosts[3], 1, 1.3, 0.0, 0.0, 0.0), // right fridge interior (large)
        Domain3D(pipeWidth, fridgeHeight, frontFridgeDepth, pipeWidthCells, fridgeHeightCells, frontFridgeDepthCells, domainsGhosts[4], 1, 1.3, 0.0, 0.0, 0.0),  // centre fridge front
        Domain3D(pipeWidth, fridgeHeight, backFridgeDepth, pipeWidthCells, fridgeHeightCells, backFridgeDepthCells, domainsGhosts[5], 1, 1.3, 0.0, 0.0, 0.0),  // centre fridge back
        Domain3D(leftFridgeWidth, fridgeHeight, frontFridgeDepth, leftFridgeWidthCells, fridgeHeightCells, frontFridgeDepthCells, domainsGhosts[6], 1, 1.3, 0.0, 0.0, 0.0),  // left fridge front
        Domain3D(leftFridgeWidth, fridgeHeight, backFridgeDepth, leftFridgeWidthCells, fridgeHeightCells, backFridgeDepthCells, domainsGhosts[7], 1, 1.3, 0.0, 0.0, 0.0),  // left fridge back
        Domain3D(rightFridgeWidth, fridgeHeight, frontFridgeDepth, rightFridgeWidthCells, fridgeHeightCells, frontFridgeDepthCells, domainsGhosts[8], 1, 1.3, 0.0, 0.0, 0.0),  // right fridge front
        Domain3D(rightFridgeWidth, fridgeHeight, backFridgeDepth, rightFridgeWidthCells, fridgeHeightCells, backFridgeDepthCells, domainsGhosts[9], 1, 1.3, 0.0, 0.0, 0.0)}; // right fridge back
    ...
    for (int i = 1; i < iterations+1; i++) // iterate updating the domain's cells
    {
        ...
        for (int domain = 0; domain < domainCount; domain++)
        {
            domains[domain].updateBoxes(domainsTransmissive[domain], timeStep); // update each domain by the allowable timestep, pointer to the transmissive domains passed
        }
